...
def _make_loop():
    ...
    with patch("nano_hermes.agent.loop.ContextBuilder"), \
         ...
         patch("nano_hermes.agent.loop.SubagentManager"):
...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from nano_hermes.command.builtin import cmd_restart
        from nano_hermes.command.router import CommandContext
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("nano_hermes.command.builtin.asyncio", new=fake_asyncio), \
             patch("nano_hermes.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("nano_hermes.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            out = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
            assert "nano_hermes" in out.content.lower() or "Model" in out.content
