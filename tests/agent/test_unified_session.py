...
from nano_hermes.command.builtin import cmd_new, register_builtin_commands
from nano_hermes.command.router import CommandContext, CommandRouter
from nano_hermes.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("nano_hermes.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        ...
        from nano_hermes.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from nano_hermes.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from nano_hermes.agent.loop import UNIFIED_SESSION_KEY
        from nano_hermes.command.builtin import cmd_stop
