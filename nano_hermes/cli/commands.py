...
from nano_hermes.cli.stream import StreamRenderer, ThinkingSpinner
from nano_hermes.config.paths import get_workspace_path, is_default_workspace
from nano_hermes.config.schema import Config
from nano_hermes.utils.helpers import sync_workspace_templates
from nano_hermes.utils.restart import (
...
def _init_prompt_session() -> None:
    ...
    from nano_hermes.config.paths import get_cli_history_path
...
def onboard(
    ...
    from nano_hermes.config.loader import get_config_path, load_config, save_config, set_config_path
    from nano_hermes.config.schema import Config
    ...
    if wizard:
        from nano_hermes.cli.onboard import run_onboard
...
def _make_provider(config: Config):
    ...
    """
    from nano_hermes.providers.factory import make_provider
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from nano_hermes.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from nano_hermes.config.paths import get_cron_dir
...
def _run_gateway(
    ...
    *,
    ...
    from nano_hermes.cron.types import CronJob
    from nano_hermes.heartbeat.service import HeartbeatService
    from nano_hermes.providers.factory import build_provider_snapshot, load_provider_snapshot
    from nano_hermes.session.manager import SessionManager
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from nano_hermes.utils.evaluator import evaluate_response
...
def channels_status(
    ...
    """Show channel status."""
    from nano_hermes.channels.registry import discover_all
    from nano_hermes.config.loader import load_config, set_config_path
...
def channels_login(
    channel_name: str = typer.Argument(..., help="Channel name (e.g. telegram, discord, matrix)"),
...
def plugins_list():
    ...
    from nano_hermes.config.loader import load_config
...
def status():
    ...
    if config_path.exists():
        from nano_hermes.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from nano_hermes.providers.github_copilot_provider import login_github_copilot
